...
Geogem.createWMSLayer = function (config) {
	...
	if (config.wmsinfoformat /*&& config.wmsinfoformat != 'none'*/ ) {
		...
		if (this.map.infoControl) {
			...
			var info = new OpenLayers.Control.WMSGetFeatureInfo({
				...
				eventListeners: {
					beforegetfeatureinfo: function () {
						...
						if (Geogem.Settings.featureInfoScreen === true) {
							featureInfoControl.vendorParams.FEATURE_COUNT = 1000;
						}
						var filterString = '';
			...
			if (isAndroid || isMobile || isIthing) {
				buffer = 20;
			} // we default to a higher buffer then OL-default (5) because of tablet use (mostly points)
...
Geogem.createWMTSLayer = function (layerConfigObj) {
	...
	var layer = new OpenLayers.Layer.WMTS({
		...
		opacity: layerConfigObj.opacity
	});
	return layer;
};
...
Geogem.init = function () {
	...
	for (i = 0; i < Geogem.Settings.baseLayers.length; i++) {
		...
		// RD nov 20313: handle blank or colored backgrounds
			...
			olLayer = new OpenLayers.Layer.XYZ(layer.title, layer.tileImage, {
				isBaseLayer: true,
				displayInLayerSwitcher: false
			});
	...
	var contextMenu = function (event, name, baselayer, clickedBaselayer, bgColor) {
		var baselayerArray = [{
			title: 'OpenTopo',
			layer: 'opentopo'
		}, {
			...
			title: 'BGT',
			layer: 'basiskaart_nieuwegein'
		}, {
			title: 'BGT_Zwart',
			layer: 'basiskaart_zwart_nieuwegein'
		}];
		var buttonName = name;
		var windowWidth = $(window).innerWidth();

		if ($('.context-menu__container')) {
			$('.context-menu__container').remove();
		}
		$('body').append('<div class="context-menu__container border-box"><div class="context-menu shadow border-box border-ng-paars"><div class="contex-menu__close border-ng-paars no-select">x</div></div></div>');
		$.each(baselayerArray, function (index, item) {
			var selected = '';
			if (name === item.title) {
				selected = 'active-baselayer';
			}
		...
		if (menu.offsetLeft + 100 > windowWidth) {
		...
		$('.context-menu__button:not(.active-baselayer)').click(function () {
			...
			if (baselayer.style['background-color'] === "rgb(255, 255, 255)") {
				baselayer.click();
			}
		...
		$('.context-menu').append('<br><div class="transparency-container"><span><b>Transparantie:</b></span><br><br><div id="trans-slider" class="border-box" data-value=""></div></div>');
		var layerTransparency = clickedBaselayer.opacity * 100;

		var $slider = $("#trans-slider").slider({
			range: false,
			min: 0,
			max: 100
		});
		...
		$slider.slider({
			values: [layerTransparency]
		});

		$('.context-menu').append('<br><div class="bg-color__container"><span><b>Achtergrond:</b></span><div>' +
			'<input id="bg-white" name="bg-color" value="bg-white" class="bg-button" type="radio"><label for="bg-white">Wit</label>' +
		...
		$('.bg-button').click(function (e) {
			...
			} else if (this.value === 'bg-white') {
				$('body').css('background-color', 'white');
			}
		});

		$('.contex-menu__close').one('click', function (e) {
			e.stopPropagation();
			$('.context-menu__container').remove();
		});
	};
	...
	$('.baselayerbutton').contextmenu(function (e) {
		...
		var bgColor = $('body').css('background-color') === "rgb(0, 0, 0)" ? 'black' : 'white';
		contextMenu(e, baselayerName, baselayer, clickedBaselayer, bgColor);
		return false;
	});
	...
	if (Geogem.Settings.legendUrl || Geogem.Settings.legend) {
		...
		if (legendSetting instanceof Array){
			// legend is an array of {title:'title',url:'url',(scale:true/false)} objects
            var setScale = '';
			$('#legend').append('<div id="legendImage" src="'+legendSetting+'"/>');
			for (i=0;i<legendSetting.length;i++){
				var l = legendSetting[i];
                var legendClass = l.title.toLowerCase().replace(' ','_') + '_lgnd';
				$('#legendImage').append('<h3 class="folded">'+l.title+'</h3><img class="legendImage ' + legendClass + '" style="display:none;" src="'+l.url+'" />');
			}
            ...
            $.each(legendSetting, function(i, l) {
                if (l.scale === true) {
                    var evtName = l.title.toLowerCase().replace(' ','_') + '_lgnd_zoom_evt';
                    var legendClass = l.title.toLowerCase().replace(' ','_') + '_lgnd';
                    Geogem.map.events.register('zoomend',evtName,function(){
                        setScale = Geogem.map.getScale();
                        scaleUrl = l.url + '&scale=' + setScale;
                        $('.' + legendClass).attr('src',scaleUrl)
                    })
                }
            })
		}
	...
	if (Geogem.Settings.infoUrl) {
		...
		if ($('#sidebar_content2_data').height() === 0) {
			$('#sidebar_content2').hide();
		}
	}
		...
		$('#sidebar').delegate('input', 'change',
			function (event) {
				...
				Geogem.downloadControl.featureAdded = function (feature) {
                    ...
                    var geometry = layer.downloadgeometry !== undefined ? layer.downloadgeometry : 'GEOM';
					...
					if (layer.params.CQL_FILTER !== undefined) {
						cql = "INTERSECTS("+geometry+", " + new OpenLayers.Format.WKT().write(feature) + ") AND (" + layer.params.CQL_FILTER.replace(/%/g, '%25') + ")";
					} else {
						cql = "INTERSECTS("+geometry+", " + new OpenLayers.Format.WKT().write(feature) + ")";
					}
	...
	Geogem.addPosition = function (x, y, name) {
		...
		if (Geogem.positionLayer === undefined) {
			...
			Geogem.positionLayer = new OpenLayers.Layer.Vector('geoposition', {
				displayInLayerSwitcher: false
			});
