...
Geogem.initWFST = function(wfs){
   ...
   if (wfsRule.filter !== "") {
	...
	Geogem.beforeFeatureModified = function(evt){
        ...
        // Get feature bounds //
        var clickedBounds = evt.feature.bounds;
        var nameKey = '';
        for (var key in Geogem.featureTypes){
            ...
            if (numberOfFeatures.length > 1) {
              ...
              $('body').append('<div class="popup__container">' +
               ...
               for (key in Geogem.featureTypes){
                ...
                var filter = evt.feature.layer.styleMap.styles.default.rules[0].filter;
                var filterField = '';
                var filterContent = '' ;
                if (filter !== '') {
                ...
                $.each(evt.object.features, function(index, item) {
                   ...
                   if (filter !== '') {
                    ...
                    if (item.bounds.intersectsBounds(clickedBounds) && item.isFiltered === true ) {
   ...
   Geogem.afterFeatureModified = function(evt){
	...
	Geogem.showFeature = function(feature){
        ...
        if (Geogem.Settings.formContainer === true) {
            if ($('.form-overlay').length === 0){Geogem.addFormContainer();}
		...
		formLocation.html('<br><div id="formulier"></div><form id="wfsform"><div class="wfsform-buttonheader">'+
            ...
            '</div></form></div>');
        if (Geogem.Settings.formContainer === true) {
            if ($('.dragbar, .resize-box').length === 0 ){$('<div class="drag-bar"></div><div class="resize-box"></div>').insertBefore('.form-body');}
			...
			for (var key in Geogem.featureTypes){
				...
				if (Geogem.featureTypes[key].TYPE == 'RADIO'){
					...
					for (var i=0;i<options.length;i++){
						htmlAppend += '<label><input type="radio" name="'+radioName+'" value="'+options[i].value+'"/> '+options[i].title+'</label>\n';
					}
                    htmlAppend += '</div>\n';
                    ...
                    var req = '';
                    var disabled = '';
                    if (Geogem.featureTypes[key].DISABLED === true) { disabled = 'disabled';}
                    if (Geogem.featureTypes[key].REQ === true) {req = 'required';}
					htmlAppend += '<div class="form-item"><span>' + Geogem.featureTypes[key].TITLE+'</span>\n';
                    ...
                    var req = '';
                    var disabled = '';
                    if (Geogem.featureTypes[key].VALUE !== undefined){value=Geogem.featureTypes[key].VALUE;}
                    if (Geogem.featureTypes[key].DISABLED === true) { disabled = 'disabled';}
                    if (Geogem.featureTypes[key].REQ === true) {req = 'required';}
                    ...
                    //console.log(d, datumNu)
                    var type = 'text';
                    if (Geogem.featureTypes[key].HIDDEN === true) {
                        htmlAppend += '<input class="form-text" type="hidden" id="form_'+key+'" value="' + datumNu + '" />';
                    } else {htmlAppend += '<div class="form-item"><span> '+title+'</span><input class="form-text" type="text" id="form_'+key+'" value="' + datumNu + '" /></div>';}
                } else if (Geogem.featureTypes[key].TYPE == 'TIJD') {
                    var title = Geogem.featureTypes[key].TITLE;
                    var d = new Date(Date.now());
                    var tijdNu = d.toISOString().substring(11, 13) + ":" + d.toISOString().substring(14, 16) + ":" + d.toISOString().substring(17, 19);
                    // console.log(d, datumNu)
                    var type = 'text';
                    if (Geogem.featureTypes[key].HIDDEN === true) {
                        htmlAppend += '<input class="form-text" type="hidden" id="form_'+key+'" value="' + tijdNu + '" />';
                    } else {htmlAppend += '<div class="form-item"><span> '+title+'</span><input class="form-text" type="text" id="form_'+key+'" value="' + tijdNu + '" /></div>';}
                } else {
    ...
    Geogem.popupCancel = function() {$('.popup__container').remove();};
    ...
    Geogem.setDrawAction = function(type) {
        if (type == 'point_icon') {Geogem.drawAction = Geogem.drawActionPoint;}
        else if (type == 'polyline_icon') {Geogem.drawAction = Geogem.drawActionPolyline;}
        else if (type == 'polygon_icon') {Geogem.drawAction = Geogem.drawActionPolygon;}
        else if (type == 'circle_icon') {Geogem.drawAction = Geogem.drawActionCircle;}
    ...
    if (Geogem.Settings.snappingTarget) {snappingTarget = Geogem.map.getLayersByName(Geogem.Settings.snappingTarget)[0];}
    ...
    window.onbeforeunload = function(){ Geogem.annuleer();};
    ...
    function dragbarLogic() {
        ...
        $('.drag-bar').on('mousedown', function(e) {
            ...
            $(document).bind('mousemove',function(e){
                ...
                $('.form-pos__container').offset({top: moveY, left: moveX});
            });
            ...
            $('.drag-bar').on( "dblclick", function() {
                $('.form-pos__container').offset({top: originalPosY, left: originalPosX});
    ...
    function checkPos(pW,pX,pY,dX,dY) {
                if (pX + 50 > dX && pY + 50 > dY) {
                    $('.form-pos__container').offset({top: dY - 50, left: dX - 50});
                } else if (pX + 50 > dX) {
                    $('.form-pos__container').offset({top: pY, left: dX - 50});
                } else if (pY + 50 > dY) {
                    $('.form-pos__container').offset({top: dY - 50, left: pX});
                } else if (pX + pW - 50 < 0) {
                    $('.form-pos__container').offset({top: pY, left: 0 - pW + 50});
                } else if (pY < 0) {
                    $('.form-pos__container').offset({top: 0, left: pX});
                }
            }
