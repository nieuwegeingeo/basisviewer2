        ...
        $('input', this.div).autocomplete(
		{
            ...
            select: function(event, ui) {
                ...
                if (ui.item) {
                    ...
                    var request = OpenLayers.Request.GET({
						url: location.protocol + '//geodata.nationaalgeoregister.nl/locatieserver/v3/lookup',
						...
						success: function(data) {
                            ...
                            if (adres.type == 'adres'){
                                $('input', self.div).val(adres.weergavenaam)
                                $('input', self.div).autocomplete( "search", adres.straatnaam + '' + adres.huisnummer );
                            }
                            else if (adres.type == 'weg'){
                                $('input', self.div).val(adres.straatnaam)
                                $('input', self.div).autocomplete( "search", adres.straatnaam );
                            }
                            else if (adres.type == 'postcode'){
                                $('input', self.div).val(adres.postcode)
                                $('input', self.div).autocomplete( "search", adres.postcode );
                            }
                            else{
...
Geogem.PdokReverseGeocoder = OpenLayers.Class({
  ...
  reverseGeocode: function(point) {
    ...
    var proxyIsNeeded = false;  // pdok has CORS headers...
    var ajax_data = {};
    var ajax_url;
    var ajax_method = 'GET';
    ...
    var options = {
      ...
      error: function(xhr, textStatus, errorThrown) {
        ...
        try {
          ...
          Geogem.PdokReverseGeocoder.prototype.onFail.apply(this, [msg]);
        }
      }
    };
    /*if (ajax_method == 'POST') {
